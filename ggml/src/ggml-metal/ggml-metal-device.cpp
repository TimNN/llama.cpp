...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_top_n_sigma(ggml_metal_library_t lib, const ggml_tensor * op) {
    // Replace with the actual enum name if it differs in your fork
    assert(op->op == GGML_OP_TOP_N_SIGMA);
    ...
    snprintf(base, 256, "kernel_top_n_sigma_%s", ggml_type_name(op->src[0]->type));
    snprintf(name, 256, "%s", base);
    ...
    res.smem = 32 * (2 * sizeof(float) + sizeof(int32_t));
    return res;
}